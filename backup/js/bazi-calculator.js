...
class BaziCalculator {
    ...
    // 计算月柱
    calculateMonthPillar(year, month, day, yearGanZhi) {
        // 简化的节气月柱计算
        // 寅月从立春开始（大约2月4日）
        const jieQiMap = [
    ...
    // 计算时柱（考虑真太阳时）
    calculateHourPillar(dayGanZhi, hour, minute, longitude) {
        // 真太阳时校正
    ...
    // 计算完整八字
    calculateBazi(birthDate, birthTime, longitude) {
        ...
        const hourResult = this.calculateHourPillar(dayPillar, hour, minute, longitude);
    ...
    calculateWuxing(bazi) {
        ...
        ['year', 'month', 'day', 'hour'].forEach(pillar => {
            ...
            if (wuxingMap[gan]) wuxingCount[wuxingMap[gan]]++;
            if (wuxingMap[zhi]) wuxingCount[wuxingMap[zhi]]++;
        });
    ...
    calculateDaYun(bazi, gender) {
        ...
        const daYun = [];
        let currentYearGanIndex = yearGanIndex;
        let currentYearZhiIndex = this.diZhi.indexOf(bazi.year.charAt(1));
    ...
    // 计算流年
    calculateLiuNian(bazi, startYear, count) {
        const liuNian = [];
        const dayGan = bazi.day.charAt(0);
        const dayGanIndex = this.tianGan.indexOf(dayGan);
